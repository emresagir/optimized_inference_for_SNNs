...
void example_usage(void) {
    ...
    // 2. Prepare input spikes (size: 3)
    // Input must be binary values: 0 (no spike) or 1 (spike)
    q7_t input_spikes[3];
    q7_t output_spikes[5];
    ...
    for (int t = 0; t < 256; t++) {
        ...
        // output_spikes contains 5 values (0 or 1)
    }
 ...
 * Input: 3 neurons (binary spikes: 0 or 1)
 * Layer 1: 5 neurons (fully connected, feedforward only, uniform params)
 * Output: 5 neurons (binary spikes: 0 or 1)
 * 
 * Total parameters: 15 feedforward weights
 * Recurrent parameters: 0 recurrent weights
 * 
 * Input Format:
