...
void example_usage(void) {
    ...
    // 2. Prepare input spikes (size: 9)
    // Input must be binary values: 0 (no spike) or 1 (spike)
    q7_t input_spikes[9];
    q7_t output_spikes[8];
    ...
    for (int t = 0; t < 256; t++) {
        ...
        // output_spikes contains 8 values (0 or 1)
    }
 ...
 * Input: 9 neurons (binary spikes: 0 or 1)
 * Layer 1: 8 neurons (fully connected, feedforward only, uniform params)
 * Output: 8 neurons (binary spikes: 0 or 1)
 * 
 * Total parameters: 72 feedforward weights
